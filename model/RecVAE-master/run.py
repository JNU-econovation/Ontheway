...
from utils import get_data, get_w_data, ndcg, recall
...
review_data = get_w_data(args.review, train_data.shape, vad_data_tr.shape, test_data_tr.shape)
review_train_data, review_valid_in_data, review_valid_out_data, review_test_in_data, review_test_out_data = review_data


def generate(batch_size, device, data_in, data_review, data_out=None, shuffle=False, samples_perc_per_epoch=1):
    ...
    for st_idx in range(0, samples_per_epoch, batch_size):
        ...
        yield Batch(device, idx, data_in, data_review, data_out)


class Batch:
    def __init__(self, device, idx, data_in, data_review, data_out=None):
        self._device = device
        self._idx = idx
...
def run(model, opts, train_data, review_data, batch_size, n_epochs, beta, gamma, dropout_rate):
    model.train()
